...
/**
 * The Bouba class represents one of the three types of pets in the CS2212 group 88 Virtual Pet Project.
 ...
 * @see Pet
 */
public class Bouba implements Pet {

    private final ElementType type = ElementType.FIRE;
    private final int vetFee = 1000;
    private int maxHealth = 50;
    private int maxSleep = 100;
    private int maxFullness = 80;
    private int maxHappiness = 200;
    ...
    private int score;
    private final int baseExp = 15;
    private int exp;
    private int level;

    private String name;
    private String[] sprites = new String[]{"Bee_Neutral.png", "Bee_Neutral2.png",
                                            "Bee_Sleep.png", "Bee_Sleep2.png",
                                            "Bee_Mad.png", "Bee_Mad2.png",
                                            "Bee_Hungry.png", "Bee_Hungry2.png",
                                            "Bee_Dead.png", "Bee_Happy.png"};
    private String currentSprite = "Bee_Neutral.png";

    /**
     * Constructor for a new file Bouba. To be used to start new games.
     */
    public Bouba() {
        this.name = "Bouba";
    ...
    /**
     * Constructor for a save file Bouba. To be used to load saved games.
     *
     * @param name The saved name of the save file Bouba.
     * @param health The saved health of the save file Bouba.
     * @param sleep The saved sleep of the save file Bouba.
     * @param fullness The saved fullness of the save file Bouba.
     * @param happiness The saved happiness of the save file Bouba.
     * @param state The saved states of the save file Bouba.
     * @param score The score of the save file.
     * @param exp The exp of the save file Bouba.
     * @param level The level of the save file Bouba.
     */
    public Bouba(String name, int health, int sleep, int fullness, int happiness,
    ...
    /**
     * {@inheritDoc}
     * Ex. Bouba is of type FIRE, so food of type FIRE is preferred, and food of type WATER is restricted.
    ...
    /**
     * {@inheritDoc}
     * Ex. Bouba is of type FIRE, so toys of type FIRE are preferred.
     * @param inventory The player's inventory.
    ...
    /**
     * {@inheritDoc}
     * Bouba's fullness decreases by 3 every update. Other stats except for health decrease by 1.
     * Bouba's health will decrease by 1 every update if fullness is 0.
     * If sleep reaches 0, Bouba's health will decrease by 20 once.
     */
    ...
    /**
     * Decreases Bouba's health by a given amount and checks for new dead state.
     */
    ...
    /**
     * Decreases Bouba's sleep by a given amount and checks for new asleep state.
     */
    ...
    /**
     * Decreases Bouba's fullness by a given amount and checks for new hungry state.
     */
    private void decreaseFullness(int FULDecr) {
        ...
        } else decreaseHealth(3);
    }

    /**
     * Decreases Bouba's happiness by a given amount and checks for new angry state.
     */
    ...
    /**
     * Increases sleep by a set amount. Bouba regenerates sleep at 4/2 seconds when asleep.
     */
     ...
     * @see Bouba#levelUp(Inventory inventory);
     */
    ...
    private void addEXP(int value, Inventory inventory) {
        exp += value * (2 * level);
        if (exp > baseExp * Math.pow(2, level - 1)) {
            levelUp(inventory);
        }
    }
     ...
     * - On even levels, Bouba's maximum health increases by 5.
     * - On odd levels, Bouba's maximum happiness increases by 5.
     * - On all levels, Bouba's maximum fullness increases by 3 and money is rewarded to the player.
     */
    private void levelUp(Inventory inventory) {
        while (exp >= baseExp * Math.pow(2, level - 1)) {
            exp -= (int) (baseExp * Math.pow(2, level - 1));
            addScore(baseExp);
            level += 1;
    ...
    /**
     * Adds score to the player. Base score values are multiplied by level, as per the difficulty level of Bouba.
     *
     * @param score The score to be added.
     */
    private void addScore(int score) {
        this.score += score * 5 * level;
    }
}
